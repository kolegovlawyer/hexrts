...
func _initialize_with_delay() -> void:
	...
	Handlers.dprint("🔄 MAP_FOG_INIT: Отложенная инициализация...")
	...
	Handlers.dprint("✅ MAP_FOG_INIT: Система тумана войны успешно инициализирована")

func _load_fog_shader() -> void:
	"""Загружает шейдер тумана войны"""
	fog_shader = load("res://shaders/fog_of_war.gdshader")
	if fog_shader:
		Handlers.dprint("📁 MAP_FOG_INIT: Шейдер тумана войны загружен")
	else:
		Handlers.dprint("❌ MAP_FOG_INIT: Ошибка загрузки шейдера тумана войны")

func _create_fog_material() -> void:
	"""Создает материал с шейдером тумана войны"""
	if not fog_shader:
		Handlers.dprint("❌ MAP_FOG_INIT: Невозможно создать материал - шейдер не загружен")
		return
	
	fog_material = ShaderMaterial.new()
	fog_material.shader = fog_shader
	
	Handlers.dprint("🎨 MAP_FOG_INIT: Материал тумана войны создан")

func _find_game_camera() -> void:
	"""Находит игровую камеру в сцене (улучшенная версия)"""
	Handlers.dprint("🔍 MAP_FOG_INIT: Поиск игровой камеры...")
	...
	if viewport_camera:
		...
		Handlers.dprint("📷 MAP_FOG_INIT: Камера найдена через viewport: ", viewport_camera.name)
		return
	...
	for path in possible_camera_paths:
		...
		if camera_node and camera_node is Camera2D:
			...
			Handlers.dprint("📷 MAP_FOG_INIT: Камера найдена по пути: ", path)
			return
	...
	if root_scene:
		...
		if game_camera:
			Handlers.dprint("📷 MAP_FOG_INIT: Камера найдена в дереве сцены: ", game_camera.name)
			return
	...
	if not all_cameras.is_empty():
		...
		Handlers.dprint("📷 MAP_FOG_INIT: Камера найдена среди всех Camera2D: ", game_camera.name)
		return
	
	Handlers.dprint("⚠️ MAP_FOG_INIT: Камера не найдена, система тумана войны может работать некорректно")
	Handlers.dprint("💡 MAP_FOG_INIT: Убедитесь что в сцене есть Camera2D узел")
...
func _setup_fog_system() -> void:
	...
	if not fog_material or not fog_overlay or not fog_manager:
		Handlers.dprint("❌ MAP_FOG_INIT: Не все компоненты найдены для настройки системы")
		return
	...
	# Частота тумана отдельно: пресеты раньше ставили 16–33 ms (слишком часто)
	fog_manager.update_frequency = 0.05
	
	Handlers.dprint("🔗 MAP_FOG_INIT: Связи системы тумана войны настроены")

func _connect_camera_signals() -> void:
	"""Подключает сигналы камеры для мгновенного обновления тумана войны"""
	if not game_camera:
		Handlers.dprint("⚠️ MAP_FOG_INIT: Камера не найдена, сигналы не подключены")
		return
	...
	if game_camera.has_signal("camera_moved"):
		...
		Handlers.dprint("📡 MAP_FOG_INIT: Подключен сигнал camera_moved")
	
	if game_camera.has_signal("camera_zoomed"):
		game_camera.camera_zoomed.connect(_on_camera_zoomed)
		Handlers.dprint("📡 MAP_FOG_INIT: Подключен сигнал camera_zoomed")
...
func _configure_fog_manager() -> void:
	...
	fog_manager.update_frequency = 0.05 # 20 Hz; не каждый кадр
	...
	Handlers.dprint("⚙️ MAP_FOG_INIT: Параметры менеджера тумана войны настроены")
